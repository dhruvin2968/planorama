...
export const Chatbot = () => {
  ...
  const [chatbotText,setChatbotText]=useState("Login To Start Chatting!!")
  useEffect(() => {
    const auth = getAuth();
    onAuthStateChanged(auth, (user) => {
      if (user) {
        setUserId(user.uid);
        setChatbotText("Need travel tips or day plans? Ask Me Anything!!");
      } else {
        setUserId(null);
        setChatbotText("Login To Start Chatting!!");
      }
    });
  }, []);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessages((prev) => [...prev, { text: 'Error: Failed to fetch response', sender: 'bot' }]);
      }
    } catch (error) {
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          {messages.length === 0 && <div className="text-center text-gray-500 mt-4">{chatbotText}</div>}
